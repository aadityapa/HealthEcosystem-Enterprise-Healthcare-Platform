...
async function bootstrap() {
  const logger = createLogger({ service: 'ehr-service' });
  ...
  const config = new DocumentBuilder()
    .setTitle('HealthEcosystem Ehr Service')
    .setDescription('ehr-service API')
    .setVersion('1.0')
    .addBearerAuth()
  ...
  const port = process.env.EHR_SERVICE_PORT ?? 3012;
