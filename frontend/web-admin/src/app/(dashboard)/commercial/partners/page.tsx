...
import type { CommercialPartner } from '@/types';

const columns: ColumnDef<CommercialPartner>[] = [
  ...
  { accessorKey: 'activeSubscriptions', header: 'Subscriptions' },
...
export default function Page() {
  const { data = [], isLoading, isError, refetch } = useQuery({
    ...
    queryFn: () => commercialApi.listPartners(),
  });

  return (
    <PageTransition>
      <div className="space-y-6">
        <PageHeader title="Commercial Partners" description={`${data.length} records`} actions={<Button><Plus className="h-4 w-4" />Add New</Button>} />
        {isError ? (
