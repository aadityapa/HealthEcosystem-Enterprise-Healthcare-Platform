...
const columns: ColumnDef<RadiologyStudy>[] = [
  ...
  { accessorKey: 'scheduledAt', header: 'Scheduled', cell: ({ row }) => new Date(row.original.scheduledAt).toLocaleString() },
...
export default function Page() {
  const { data = [], isLoading, isError, refetch } = useQuery({
    ...
    queryFn: () => radiologyApi.listStudies(),
  });

  return (
    <PageTransition>
      <div className="space-y-6">
        <PageHeader title="Imaging Studies" description={`Scheduled and completed studies ${data.length} records`} actions={<Button><Plus className="h-4 w-4" />Add New</Button>} />
        {isError ? (
