...
const columns: ColumnDef<AiAgentRecord>[] = [
  ...
  { accessorKey: 'capabilities', header: 'Capabilities' },
...
export default function Page() {
  const { data = [], isLoading, isError, refetch } = useQuery({
    queryKey: ['agents-patient'],
    queryFn: () => agentsApi.listAgentsByType('patient'),
  });

  return (
    <PageTransition>
      <div className="space-y-6">
        <PageHeader title="Patient Agents" description={`${data.length} records`} actions={<Button><Plus className="h-4 w-4" />Add New</Button>} />
        {isError ? (
