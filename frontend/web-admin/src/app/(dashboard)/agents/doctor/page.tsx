...
export default function Page() {
  const { data = [], isLoading, isError, refetch } = useQuery({
    queryKey: ['agents-doctor'],
    queryFn: () => agentsApi.listAgentsByType('doctor'),
  });

  return (
    <PageTransition>
      <div className="space-y-6">
        <PageHeader title="Doctor Agents" description={`${data.length} records`} actions={<Button><Plus className="h-4 w-4" />Add New</Button>} />
        {isError ? (
