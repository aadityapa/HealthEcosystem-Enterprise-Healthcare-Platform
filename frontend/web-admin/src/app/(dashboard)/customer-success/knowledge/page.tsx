...
const columns: ColumnDef<KnowledgeArticle>[] = [
  { accessorKey: 'articleCode', header: 'Code', cell: ({ row }) => <span className="font-mono text-sm">{row.original.articleCode ?? '—'}</span> },
...
export default function Page() {
  const { data = [], isLoading, isError, refetch } = useQuery({
    queryKey: ['cs-knowledge'],
    queryFn: () => customerSuccessApi.listKnowledge(),
  });

  return (
    <PageTransition>
      <div className="space-y-6">
        <PageHeader title="Knowledge Base" description={`${data.length} records`} actions={<Button><Plus className="h-4 w-4" />Add New</Button>} />
        {isError ? (
