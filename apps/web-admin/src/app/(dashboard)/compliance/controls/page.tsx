...
const columns: ColumnDef<ComplianceControl>[] = [
  { accessorKey: 'controlCode', header: 'Code', cell: ({ row }) => <span className="font-mono text-sm">{row.original.controlCode ?? '—'}</span> },
  ...
  return (
    <PageTransition>
      <div className="space-y-6">
        <PageHeader title="Compliance Controls" description={`${data.length} records`} actions={<Button><Plus className="h-4 w-4" />Add New</Button>} />
        {isError ? (
