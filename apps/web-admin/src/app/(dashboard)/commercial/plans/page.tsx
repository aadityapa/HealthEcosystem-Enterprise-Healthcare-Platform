...
const columns: ColumnDef<CommercialPlan>[] = [
  { accessorKey: 'planCode', header: 'Code', cell: ({ row }) => <span className="font-mono text-sm">{row.original.planCode ?? '—'}</span> },
  { accessorKey: 'name', header: 'Name' },
  { accessorKey: 'tier', header: 'Tier', cell: ({ row }) => <GenericStatusBadge value={String(row.original.tier)} /> },
  { accessorKey: 'monthlyPrice', header: 'Monthly', cell: ({ row }) => formatCurrency(row.original.monthlyPrice) },
...
export default function Page() {
  const { data = [], isLoading, isError, refetch } = useQuery({
    ...
    queryFn: () => commercialApi.listPlans(),
  });

  return (
    <PageTransition>
      <div className="space-y-6">
        <PageHeader title="Commercial Plans" description={`${data.length} records`} actions={<Button><Plus className="h-4 w-4" />Add New</Button>} />
        {isError ? (
