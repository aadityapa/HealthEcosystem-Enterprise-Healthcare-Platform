...
const columns: ColumnDef<AutomationRule>[] = [
  { accessorKey: 'ruleCode', header: 'Code', cell: ({ row }) => <span className="font-mono text-sm">{row.original.ruleCode ?? '—'}</span> },
  { accessorKey: 'name', header: 'Name' },
  { accessorKey: 'trigger', header: 'Trigger' },
  { accessorKey: 'action', header: 'Action' },
  { accessorKey: 'executionsToday', header: 'Runs Today' },
  { accessorKey: 'status', header: 'Status', cell: ({ row }) => <GenericStatusBadge value={String(row.original.status)} /> },
];

export default function Page() {
  const { data = [], isLoading, isError, refetch } = useQuery({
    queryKey: ['workflow-automation'],
    queryFn: () => workflowApi.listAutomationRules(),
  });

  return (
    <PageTransition>
      <div className="space-y-6">
        <PageHeader title="Automation Rules" description={`${data.length} records`} actions={<Button><Plus className="h-4 w-4" />Add New</Button>} />
        {isError ? (
