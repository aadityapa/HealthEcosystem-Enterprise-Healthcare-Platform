...
const columns: ColumnDef<WorkflowDefinition>[] = [
  { accessorKey: 'definitionCode', header: 'Code', cell: ({ row }) => <span className="font-mono text-sm">{row.original.definitionCode ?? '—'}</span> },
  ...
  return (
    <PageTransition>
      <div className="space-y-6">
        <PageHeader title="Workflow Definitions" description={`${data.length} records`} actions={<Button><Plus className="h-4 w-4" />Add New</Button>} />
        {isError ? (
