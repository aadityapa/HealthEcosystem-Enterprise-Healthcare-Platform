...
const columns: ColumnDef<Geofence>[] = [
  ...
  { accessorKey: 'activeCollections', header: 'Active' },
  { accessorKey: 'status', header: 'Status', cell: ({ row }) => <GenericStatusBadge value={String(row.original.status)} /> },
];

export default function Page() {
  const { data = [], isLoading, isError, refetch } = useQuery({
    queryKey: ['field-geofences'],
    queryFn: () => fieldApi.listGeofences(),
  });

  return (
    <PageTransition>
      <div className="space-y-6">
        <PageHeader title="Geofences" description={`Collection zone boundaries ${data.length} records`} actions={<Button><Plus className="h-4 w-4" />Add New</Button>} />
        {isError ? (
