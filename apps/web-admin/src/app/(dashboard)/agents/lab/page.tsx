...
export default function Page() {
  const { data = [], isLoading, isError, refetch } = useQuery({
    queryKey: ['agents-lab'],
    queryFn: () => agentsApi.listAgentsByType('lab'),
  });

  return (
    <PageTransition>
      <div className="space-y-6">
        <PageHeader title="Lab Agents" description={`${data.length} records`} actions={<Button><Plus className="h-4 w-4" />Add New</Button>} />
        {isError ? (
