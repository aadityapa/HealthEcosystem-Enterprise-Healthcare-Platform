...
async function bootstrap() {
  const logger = createLogger({ service: 'crm-service' });
  ...
  const config = new DocumentBuilder()
    .setTitle('HealthEcosystem Crm Service')
    .setDescription('crm-service API')
    .setVersion('1.0')
    .addBearerAuth()
  ...
  const port = process.env.CRM_SERVICE_PORT ?? 3011;
